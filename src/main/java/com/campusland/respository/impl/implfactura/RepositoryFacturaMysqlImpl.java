...
public class RepositoryFacturaMysqlImpl implements RepositoryFactura {
    ...
    private static final String SQL_GET_TOTAL_SELLS = "SELECT SUM(importe) as total_ventas FROM item_factura";

    private static final String SQL_GET_TOTAL_TAXES = "SELECT SUM(iva) as total_impuestos FROM item_factura";
    ...
    public int totalVentas() {
        int totalVentas = 0;
        try (Connection conn = getConnection();
             PreparedStatement preparedStatement = conn.prepareStatement(SQL_GET_TOTAL_SELLS);
             ResultSet rs = preparedStatement.executeQuery()) {
    ...
    public int totalImpuestos() {
        ...
        try (Connection conn = getConnection();
             PreparedStatement preparedStatement = conn.prepareStatement(SQL_GET_TOTAL_TAXES);
             ResultSet rs = preparedStatement.executeQuery()) {

            while (rs.next()) {
                totalImpuestos = rs.getInt("totalIva");
            }
    ...
    private Factura creaFactura(ResultSet rs) throws SQLException {
        ...
        final Cliente cliente = new Cliente(documento, nombre, apellido, email, direccion, celular);
